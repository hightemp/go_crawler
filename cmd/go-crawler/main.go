...
type Job struct {
	...
	MaxDepth      int      `yaml:"max_depth"` // 0 means unlimited
	MaxPages      int      `yaml:"max_pages"` // 0 means unlimited
}
...
type assetRef struct {
	...
	// srcset handling
	isSrcset bool
	desc     string // descriptor for srcset item (e.g., "1x", "480w")
}
...
func runJob(job Job, client *http.Client, ua string, retries int, backoffMs int) error {
	...
	case "pages", "site":
		// For site default to sameHostOnly if not specified (but we cannot distinguish unset bool),
		// so we keep user's value; recommendation: set same_host_only: true for site in config.
...
func setNodeAttrValue(n *html.Node, attrIndex int, newValue string, isSrcset bool, descriptor string) {
	...
	// Simpler approach: keep a tiny state by attaching temporary map to node via a fake attribute is heavy.
	// Pragmatic approach: if descriptor present, replace first entry with same descriptor; otherwise replace first entry.
